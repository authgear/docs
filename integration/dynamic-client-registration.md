---
description: >-
  Let OAuth clients, such as AI agents' MCP clients, register themselves with
...
* **Open registration:** Turning the IAT requirement off lets anyone register a client with your project. This is what the MCP flow expects, but combine it with a client limit.

## Enable DCR in the Portal <a href="#enable-dcr-in-the-portal" id="enable-dcr-in-the-portal"></a>

### 1. Turn on registration

In the Portal, go to **Applications** → the **Dynamic clients (DCR)** tab. Turn on **Enable dynamic client registration** and press **Save**. The tab then shows your project's registration endpoint.
...
* Keep it on and press **Create token** to mint an IAT. Pick the token type (**Third-party** or **First-party**) and an expiry. The token value is shown once, together with a ready-to-run example request — copy it and store it securely.
...
The **Default client configuration** card sets the token lifetimes (access token, refresh token, refresh token idle timeout) applied to every dynamically registered client, in case the DCR cohort needs stricter settings than your own apps.
...
* On each scope of that resource, tick **Allow dynamic third-party clients to request this scope**.
...
The user signs in and, for a third-party client, approves the consent screen. The client exchanges the code at `/oauth2/token` (including the same `code_verifier`, `redirect_uri` and `resource`), and the issued access token carries the resource URI in its `aud` claim, which your API validates.
