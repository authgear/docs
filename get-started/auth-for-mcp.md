...
The end-to-end flow, all of it automatic once set up:
...
4. The client runs the Authorization Code Flow with PKCE, passing `resource=<your MCP server URI>`. The user signs in with Authgear and approves the consent screen.
...
* On each scope MCP clients should be able to request, tick **Allow dynamic third-party clients to request this scope**.

### 3. Enable Dynamic Client Registration with open registration

Go to **Applications** → the **Dynamic clients (DCR)** tab:

* Turn on **Enable dynamic client registration** and press **Save**.
...
{% hint style="info" %}
Open registration means anyone can register a client with your project. Registered clients are still harmless until a real user signs in and consents, and they can only request the resources and scopes you opened in step 2.
{% endhint %}
...
4. Check `aud` includes your MCP server's URI (`https://mcp-server.example.com`) — this is what stops a token issued for another audience from being replayed against your server.
5. Check `exp` has not elapsed, and that the token's `scope` covers the requested tool.
...
* Users manage their consents like any other third-party app; the consent screen shows the scopes the MCP client requested.
